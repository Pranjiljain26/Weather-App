...
def data_get():
    ...
    city= city_name.get()
    data=requests.get("https://api.openweathermap.org/data/2.5/weather?q="+city+"&appid=c8bcca3f3721d49b2e534724df73e65a").json()
...
#creating the combo bok by ttk
com=ttk.Combobox(win,text="Pranjil_Weather_App",values=list_name,font=("Time New Roman",20,"bold"),textvariable=city_name)
...
#the duplicate labels are named as label1 for getting data from the internet
#here we change th ex axis only and not the y axis
...
#placing a get weather button(used Button parameter for Button)
